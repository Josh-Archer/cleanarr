...
def _is_http_event(event):
    ...
    return any(
        key in event
        for key in ("body", "rawPath", "rawQueryString", "requestContext", "httpMethod")
    )
...
def lambda_handler(event, context):
    try:
        logger.info("Starting Lambda execution for Cleanarr")
        logger.info(
            f"Event keys: {list(event.keys()) if isinstance(event, dict) else event}"
        )
        ...
        if records:
            ...
            if failed_message_ids:
                logger.warning(
                    f"Queued webhook processing reported failures: {queue_summary}"
                )
            else:
        ...
        if _is_http_event(event):
            logger.info(
                "Received direct webhook invocation; dispatching through webhook app"
            )
            return _http_response_from_event(event)

        # EventBridge schedule / manual invoke: full library cleanup.
        # Downstream homelab wiring often uses one function for both SQS ESM and
        # scheduled full-library runs (ECR legacy name cleanarr-job).
        return _run_scheduled_cleanup()
